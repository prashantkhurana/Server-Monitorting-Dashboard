<template>
    <div class="card material-table">
        ...
        <appBidderDetailsModal :selectedBidder="selectedBidder" :open.sync="open"></appBidderDetailsModal>
        <table ref="table">
            <thead>
                <tr>
                    <th v-for="(column, index) in columns" @click="sort(index)" :class="(sortable ? 'sorting ' : '')
							+ (sortColumn === index ?
								...
								: '')
							+ (column.numeric === 'true' ? ' numeric' : '')" :style="{width: column.width ? column.width : 'auto'}">
                        {{column.label}}
                    </th>
            ...
            <tbody>
                <tr v-for="(row, index) in paginated" :class="onClick ? 'clickable' : ''">
                    <td v-for="column in columns" :class="column.numeric === 'true'? 'numeric' : ''" v-if="notHtmlOrJson(column)">
                        {{ collect(row, column.field) }}
                    </td>
                    <td v-for="column in columns" :class="column.numeric === 'true' ? 'numeric' : ''" v-html="collect(row, column.field)" v-if="column.html">
                    </td>
                    <td v-for="column in columns" :class="column.numeric === 'true' ? 'numeric' : ''" v-if="column.json">
                        <a class="waves-effect waves-light btn" :class="{red : row.overallStatus === 'RED_FLAG', yellow : row.overallStatus === 'YELLOW_FLAG', green : row.overallStatus === 'GREEN_FLAG'}"
                            @click="openModal(row)">Details</a>
        ...
        <div class="table-footer" v-if="paginate">
            ...
            <div class="datatable-info">
                {{(currentPage - 1) * currentPerPage ? (currentPage - 1) * currentPerPage : 1}} -{{Math.min(processedRows.length, currentPerPage
                * currentPage)}} of {{processedRows.length}}
            </div>
            <div>
                <ul class="material-pagination">
                    <li>
                        <a class="waves-effect btn-flat" tabindex="0" @click.prevent="previousPage">
							<i class="material-icons">chevron_left</i>
						</a>
                    </li>
                    <li>
                        <a class="waves-effect btn-flat" tabindex="0" @click.prevent="nextPage">
...
<script>
    ...
    import BidderDetails from '../BidderDetails.vue'
    import BidderDetailsModal from './BidderDetailsModal.vue'
    ...
    export default {
        ...
        components: {
            appBidderDetails: BidderDetails,
            appBidderDetailsModal : BidderDetailsModal
        },
        ...
        methods: {
            ...
            openModal(row) {
                this.currentBidderHealth = row;
                this.selectedBidder = row.hostName;
                this.open = !this.open;
            },
            onTableLength: function (e) {
                this.currentPerPage = e.target.value;
            },
            notHtmlOrJson: function (column) {
                return !column.html && !column.json
            },
            sort: function (index) {
                if (!this.sortable)
                    return;
                ...
                if (this.sortable !== false)
                    computedRows = computedRows.sort((x, y) => {
                        ...
                        const cook = (x) => {
                            ...
                            if (typeof (x) === 'string') {
                                x = x.toLowerCase();
                                if (this.columns[this.sortColumn].numeric === 'true')
